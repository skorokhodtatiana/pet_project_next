...
import { createRecipe, deleteRecipes, getRecipes, updateRecipes } from "../actions/recipe";
...
export const useRecipeStore = create<IRecipeState>((set) => ({
	...
	updateRecipe: async(id: string, formData: FormData) => {
		...
		try {
			const result = await updateRecipes(id, formData);
